...
const qwe= new XMLHttpRequest();
qwe.onload = function() {
    const data = JSON.parse(this.responseText);
    console.log(data);
    var randomNumber = Math.floor(Math.random() * 100);
    const body1Id = document.getElementById('body1Id');
    const body1Body = document.getElementById('body1Body');
    

    body1Id.textContent=data[randomNumber].id;
    body1Body .textContent=data[randomNumber].body;
    


    randomNumber = Math.floor(Math.random() * 100);
    const body2Id = document.getElementById('body2Id');
    const body2Body = document.getElementById('body2Body');

    body2Id.textContent=data[randomNumber].id;
    body2Body .textContent=data[randomNumber].body;


    randomNumber = Math.floor(Math.random() * 100);
    const body3Id = document.getElementById('body3Id');
    const body3Body = document.getElementById('body3Body');

    body3Id.textContent=data[randomNumber].id;
    body3Body .textContent=data[randomNumber].body;


}
qwe.onerror = function() {
    console.log("ERROR!!!!");
    length.log(this);
}

qwe.open("GET","https://jsonplaceholder.typicode.com/posts?utm_source=Mailerlite&utm_medium=E-mail&utm_campaign=Test%20Series&utm_term=2022-08-11");
qwe.send();